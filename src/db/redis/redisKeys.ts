export const redisKeys = {
  ...
  meta: (pin: string) => `session:${pin}:meta`,

  // Hash { questionText, answerOptions, correctIndexes, timeLimit, scoringWeight }
  question: (pin: string, qIdx: number) => `session:${pin}:question:${qIdx}`,
  ...
  // Set of userId
  answered: (pin: string, qIdx: number) => `session:${pin}:answers:${qIdx}`,

  // Hash { indexes, answeredAt }
  answer: (pin: string, qIdx: number, userId: string) =>
    `session:${pin}:question:${qIdx}:player:${userId}`,

  // Sorted Set
  leaderboard: (pin: string) => `session:${pin}:leaderboard`,
};
